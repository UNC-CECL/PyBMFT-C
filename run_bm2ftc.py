...
for time_step in range(int(bmftc_ML.dur)):
    ...
    # Set fetch, depth
    # delta_fetch_ML = bmftc_ML.fetch[bmftc_ML.startyear + time_step] - bmftc_ML.fetch[bmftc_ML.startyear + time_step - 1]  # [m] Calculate change in fetch from erosion of mainland marsh
    # delta_fetch_BB = bmftc_BB.fetch[bmftc_BB.startyear + time_step] - bmftc_BB.fetch[bmftc_BB.startyear + time_step - 1]  # [m] Calculate change in fetch from erosion of back-barrier marsh
    ...
    x_b_TS_ML[time_step] = bmftc_ML.x_b
    x_b_TS_BB[time_step] = bmftc_BB.x_b

    bmftc_ML._bfo = bmftc_ML.bfo - bmftc_ML.x_b
    bmftc_BB._bfo = bmftc_BB.bfo - bmftc_BB.x_b

    bmftc_ML.fetch[bmftc_ML.startyear + time_step - 1] = bmftc_ML.bfo
    bmftc_BB.fetch[bmftc_BB.startyear + time_step - 1] = bmftc_BB.bfo

    print()
    print(" Fetch diff:", bmftc_ML.bfo - bmftc_BB.bfo)
    print("delta_fetch_ML:", delta_fetch_ML)
    print("delta_fetch_BB:", delta_fetch_BB)
