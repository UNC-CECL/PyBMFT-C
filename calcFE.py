...
def calcFE(bfoc, bfop, elevation, yr, organic_dep_autoch, organic_dep_alloch, mineral_dep, rhos):
    ...
    x_m1 = math.ceil(bfop)  # First marsh cell to erode

    x_m2 = math.ceil(bfoc)  # Last  marsh cell to erode
    ...
    if E <= 0:
        ...
        if elevation[pyr, 0] < elevation[0, x_m1]:  # If depth of erosion is below the lowest marsh deposit
            ...
            FE_org = np.sum(organic_dep[0: pyr, x_m1]) * E  # [g] OM eroded is equal to the total amount of OM in the eroding marsh edge (including both initial deposit and OM
            # deposited since the model run began) times the fraction of the marsh edge cell that is eroded
            FE_min = np.sum(mineral_dep[0: pyr, x_m1]) * E + usmass  # [g] MIN eroded is equal to the total amount of OM in the eroding marsh edge (including both initial
            ...
            boundyr = bisect.bisect_left(elevation[:, x_m1], elevation[0, pyr])  # Year at which deposit is above the boundary for the depth of erosion
            FE_org = np.sum(organic_dep[boundyr: pyr, x_m1]) * E  # [g] Total mass of OM deposited in the marsh edge cell
            FE_min = np.sum(mineral_dep[boundyr: pyr, x_m1]) * E  # [g] Total mass of MIN deposited in the marsh edge cell
    else:
        ecells = x_m2 - x_m1  # Number of cells eroded
        Hfrac_ero = np.ones(ecells)
        Hfrac_ero[0] = x_m1 - bfop  # Horizontal fraction of previous marsh edge that is eroded
